...
- **`[Local]`** — Uses a local XML file at `.agents/plans/plan-{N}-tasks.xml`. No external services required. Requires the plugin's SessionStart hook to have run (installs `fast-xml-parser` and compiles the task scripts into `${shipsmooth.dist.path}/`).
...
`[Local]` Script invocations use `node ${shipsmooth.dist.path}/<script>.js`. All scripts read/write `.agents/plans/plan-{N}-tasks.xml` relative to the repo root.
...
- `[Local]` Run `node ${shipsmooth.dist.path}/update-status.js --plan {N} --task {id} --status de-risked` and `node ${shipsmooth.dist.path}/add-comment.js --plan {N} --task {id} --message "De-risk draft ready for review"`.
...
- `[Local]` Run `node ${shipsmooth.dist.path}/update-status.js --plan {N} --task {id} --status agent-coded` and `node ${shipsmooth.dist.path}/set-commit.js --plan {N} --task {id} --commit $(git rev-parse HEAD)`.
...
4. Commit the completed task (tests + implementation):
   ...
   - `[Local]` Run `node ${shipsmooth.dist.path}/update-status.js --plan {N} --task {id} --status agent-coded` and `node ${shipsmooth.dist.path}/set-commit.js --plan {N} --task {id} --commit $(git rev-parse HEAD)`. No draft review needed.
...
- **Minor deviation** (task split, reorder, clarification):
  ...
  - `[Local]` Run `node ${shipsmooth.dist.path}/add-deviation.js --plan {N} --task {id} --type minor --message "..."`, continue.
- **Major deviation** (fundamental plan problem, architecture issue, blocked): Stop immediately.
  ...
  - `[Local]` Run `node ${shipsmooth.dist.path}/project-update.js --plan {N} --blocked --message "..."`.
...
- `[Local]` Run `node ${shipsmooth.dist.path}/project-update.js --plan {N} --status complete --message "Plan complete."`. Commit the final XML state. Update the permanent backlog feature issue (if tracked externally) or note delivery in the plan file.
...
- `[Local]` Run `node ${shipsmooth.dist.path}/update-status.js --plan {N} --task {id} --status needs-triage` for each unresolved task. Run `node ${shipsmooth.dist.path}/project-update.js --plan {N} --status in-review --message "..."`. Commit the XML. Wait for human to review.
...
- `[Local]` Run `node ${shipsmooth.dist.path}/project-update.js --plan {N} --status abandoned --message "Superseded by plan-{M}."`. Commit the final XML state.
